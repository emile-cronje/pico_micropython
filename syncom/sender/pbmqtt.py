...
class MQTTlink:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # SynCom string mode
        print("create channel...wdog" + str(wdog))
        self.channel = SynCom(False, d['sckin'], d['sckout'], d['srx'], d['stx'],
                              None, wdog, True, self.verbose)
...
# After acquiring sync, start() gets rescheduled.
    async def start(self, channel):
        self.verbose and print('pbmqtt Starting...')
        ...
        if self.lw_topic is not None:
            msg = argformat(WILL, self.lw_topic, self.lw_msg, self.lw_retain, self.lw_qos)
            print("pbmqtt channel send WILL..." + str(msg))            
            channel.send(msg)                        
            res = await channel.await_obj()
            print("pbmqtt channel sent WILL..." + str(res))
