#Channel mqtt.py MQTT library for the micropython board using an ESP8266.
# asyncio version

# Author: Peter Hinch
# Copyright Peter Hinch 2017-2021 Released under the MIT license

# Accessed via pbmqtt.py on Pico
...
# Task runs continuously. Process incoming Pico messages.
# Started by main_task() after client instantiated.
    async def from_pico(self):
        client = self.client
...
# Runs when channel has synchronised. No return: Pico resets ESP on fail.
# Get parameters from Pico. Process them. Connect. Instantiate client. Start
# from_pico() task. Wait forever, updating connected status.
    async def main_task(self, _):
        ...
        while not got_params:
            ...
            ilst = istr.split(SEP)
            command = ilst[0]
            print("await_obj returned, got command: " + istr)            
        ...
        try:
            ...
            # Cause Pico to reboot us when application requires it.
        ...
        # Set channel running
        asyncio.create_task(self.from_pico())
